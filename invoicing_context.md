...
* (1) Prefer `externalReference` if possible. In order to recognize future events regarding the created `Order` and to `Update`, using `externalReference` is perfectly fine. `reference` will override the reference generated by GREM by default. If you use `reference`, it is your responsability to prevent future clashes with the database sequences used by GREM to generate references. Must be unique with respect to `Invoice.Order`.
...
&nbsp;&nbsp;`preaccountingReference` | `string` | optional preaccounting reference
...
&nbsp;&nbsp;`iban` | `string` | ESR Payments are deprecated, use `swissQrCode` instead. iban for the payment (optional); always pass the iban or swissQrCodeText, if you pass the iban field esr attributes and swissQrCodeText must be empty or omitted
&nbsp;&nbsp;`esrReference` | `string` | ESR Payments are deprecated, use `swissQrCode` instead. reference number on the payment slip. Pass this number together with the esrParticipantNumber if you don't have an iban or swissQrCodeText
